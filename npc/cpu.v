...
reg [31: 0] read_rs1_data; // 寄存器 rs1的数据
...
controller CONTROLLER(
    ...
    .extOP(extOP),
    .pcImm_NEXTPC_rs1Imm(pcImm_NEXTPC_rs1Imm),
    .ebreak_or_ecall(ebreak_or_ecall)
);
...
import "DPI-C" function void exit_m();
import "DPI-C" function void set_npc_state(int halt_ret);

	always @(posedge clk) begin
		if(ebreak_or_ecall) begin
			exit_m();
            $display("rs1_p : %d , read_rs1_data :  %d\n",rs1_p, read_rs1_data);
            set_npc_state(read_rs1_data);
		end
	end
